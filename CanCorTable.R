#' APA Canonical Correlation Table
#'
#' APA canonical correlation tables are created from data frames using CanCorTable
#' @param x Variable x.
#' @return APA canonical correlation table
#' @export
...
CanCorTestTable <- function(x) {
  if (class(x) != "cancor") return("Object must be of class cancor from the candisc package.")
  k <- length(x$cancor)
  cbind(Variate = 1:k, candisc::Wilks(x)) %>%
